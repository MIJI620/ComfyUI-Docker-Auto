...
基于 `ComfyUI_windows_portable_nvidia`（v0.34 便携包）同一运行配置，用 `ComfyUI-master`（v0.34.0 源码）
构建的 **NVIDIA GPU Docker 镜像**，已通过本地端到端测试（build / 启动 / HTTP / 认证开·关 / preset 全绿）。

- **Python 3.13**、**PyTorch 2.13.0**（GPU=cu130 / CPU）与便携包版本对齐
- **CPU 与 GPU 可切换**：笔记本无独显用 CPU，服务器 RTX 用 cu130
- **支持自选版本来源**：在线拉取（`remote-stage`，GitHub）/ 本地目录（`local-stage`，`versions/<版本>/`）
- **国内网络友好**：base 镜像 `BASE_IMAGE` 可换国内源；**torch / PyPI 下载源用 `net_lib.sh` 真实嗅探自动选取**（GPU 优先阿里云 `pytorch-wheels/cu130`，全部不可达才回退官方）
- **支持自选启动方式**：预设表（gpu/cpu/lowvram/fp16…），端口与性能参数各自独立
- **认证与性能模式解耦**：每个预设可独立开启/关闭 Nginx BasicAuth，并可限定允许的用户白名单
...
```
ComfyUI-testkit/                  # 本测试包；等价于发布包 ComfyUI-docker/ 的布局
├── README.md                     # 本教程
├── HELPER.md                     # 命令行速查
...
│   └── 0.34.0/                   #   ComfyUI 源码
└── app/docker/                   # ★ 与发布包 ComfyUI-docker/app/docker 逐字一致
...
> **构建路径**：Dockerfile 位于 `app/docker/Dockerfile`（不是旧文档里的 `docker/Dockerfile`）。构建时请在 `ComfyUI-testkit/` 根目录执行，用 `-f app/docker/Dockerfile`。
...
### 位置约定（重要）
- **`docker build`**：在**包含 `ComfyUI-testkit/` 或 `ComfyUI-docker/` 的上级根目录**执行（构建上下文 = 上级根，用 `-f ComfyUI-testkit/app/docker/Dockerfile` 或 `-f ComfyUI-docker/app/docker/Dockerfile`）。
- 本测试包内：`cd ComfyUI-testkit`，用 `-f app/docker/Dockerfile`。
...
docker build --target local-stage \
  ...
  -f app/docker/Dockerfile -t comfyui-docker:latest .
```
...
docker build --target local-stage \
  ...
  --build-arg BASE_IMAGE=docker.m.daocloud.io/library/python:3.13-slim \
  -f app/docker/Dockerfile -t comfyui-test:cpu .
```
...
```
- compose 默认 `runtime: nvidia`、`target: local-stage`、`PRESET=gpu`、`EXPOSE_PORT=8189`、映射 `8189:8189`。
- 换远程源码：改 compose 的 `target` 为 `remote-stage`。
...
- HTTPS 时长：nginx 的 `ssl_certificate` / `ssl_certificate_key` 指向这两个文件；证书续期后重启容器生效。
...
## 五、网络源自动选择（net_lib.sh，重要）

构建镜像时所有 **PyPI / torch 下载源** 都由统一的 `net_lib.sh` 决定，不搞任何特例：

- 判定一个"源"是否可用，只看它**能否真实返回 HTTP 200** 到需要下载的文件（`pick_first_200` 逐源真实嗅探）。
- HTTP 3xx(重定向/302) 视为"该源并未持有文件"，不算合法持源（不跟随重定向）。
- HTTP 4xx/5xx / 超时 / 不可达 → 继续下一个；候选按"国内优先"排序，官方源只在列表最后作兜底。
- **PyPI 全局源**：先探测 `torch/ torchvision/ torchaudio/` 三个索引页是否 200，选中的写入 `pip config`。
- **torch**：GPU 分支逐源探测 `cu130` 三个 wheel 的 HTTP 200，命中才用作 `--index-url`；CPU 分支走标准 PyPI simple。
- 构建日志会打印 `Using PyPI index: ...` / `torch CUDA index: ...`，据此确认真实命中的源。

> 超时可调：`NET_LIB_TOTAL_TIMEOUT`（默认 20s）、`NET_LIB_CONNECT_TIMEOUT`（默认=整体）两个环境变量，构建前 `export` 覆盖即可。
...
docker run ... \
  ...
  -v /path/to/comfyui.json:/config/comfyui.json:ro ...
```
compose 已内置 `./models ./input ./output ./user` 的相对挂载。
...
本方案配套测试入口 `ComfyUI-testkit/`（`run_test.bat` 管理员运行）跑通 7 阶段：
...
其中认证**开/关是两个独立测试**；`[6/7]` 是 bcrypt 全量自测。逻辑层有 `tests/test_logic.py`（不依赖 Docker，开发机可跑）。
本包 `app/docker/` 与发布包 `ComfyUI-docker/app/docker/` **逐字一致**：上述测试通过 = 本镜像同套逻辑通过。
...
- 上线前：**必须把默认 admin 的 bcrypt hash 换成你的真实密码生成值**（见上文）。
- 不要用 Windows 便携版的 `--windows-standalone-build` 参数。
- remote 模式 clone 需访问 GitHub（可改为镜像站）。
