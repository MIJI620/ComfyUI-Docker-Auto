...
> 完整说明见 README.md。
> **当前目录约定**：`docker build` 在 `ComfyUI-testkit/`（本包）或含 `ComfyUI-docker/` 的上级根目录执行，用 `-f app/docker/Dockerfile`；`docker compose` 在包目录内执行。
...
### 在线拉取远程版本源码（remote-stage，需访问 GitHub）
```bash
cd ComfyUI-testkit
...
### 无认证预设(cpu-noauth) / 切换 preset / 附加参数
```bash
...
## docker-compose（默认 remote/本地源切换）
```bash
cd ComfyUI-testkit
...
- 换本地源码：把 compose 的 `target` 改为 `local-stage`（确保 `versions/<版本>/` 在）。
